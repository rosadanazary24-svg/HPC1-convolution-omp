...
#include <sys/time.h>
#include <time.h>
#include <omp.h> // Because it is essential library for OpenMP Parallelization
               ...
               float* kernel, int kernelSizeX, int kernelSizeY)
{
    ...
    // start convolution
  //I have choosen to parallelize at the outer loop (i,j) because
  // Each pixel (i,j) are independent there for there is no data dependencies
 //collapse (2) distributes both loops across threads for better load balancing
 //schedule(static) chosen due to uniform workload per iteration
